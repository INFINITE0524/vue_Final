...
async function checkToken() {
  try {
    //取得token
    token.value = document.cookie.replace(/(?:(?:^|.*;\s*)maxToken\s*\=\s*([^;]*).*$)|^.*$/, '$1')
    //進行token的驗證
...
<template>
  ...
  <div id="todoListPage" class="bg-half">
    <nav>
      ...
      <ul>
        <li class="todo_sm">
          <a href="javascript:;"><span>{{ currentUser ? `${currentUser.nickname}的代辦` : '尚未登入' }}</span></a>
        </li>
      ...
      <div class="todoList_Content">
        ...
        <div class="todoList_list" v-if="todoLists.length > 0">
          <ul class="todoList_tab">
            <li>
              <a href="#" @click.prevent="swtichFilter()" :class="{ active: filterStatus == null ? true : false }">全部</a>
            </li>
            <li>
              <a href="#" @click.prevent="swtichFilter(false)" :class="{ active: filterStatus == false ? true : false }">待完成</a>
            </li>
            <li>
              <a href="#" @click.prevent="swtichFilter(true)" :class="{ active: filterStatus == true ? true : false }">已完成</a>
            </li>
          </ul>
          <div class="todoList_items">
            <ul class="todoList_item">
              <li v-for="(item) in todoLists" :key=item.id v-show="filterStatus == null ? true : filterStatus == item.status">
                <label class="todoList_label">
                  <input class="todoList_input" type="checkbox" value="true" :checked="item.status" @click.prevent="updateToggle(item.id)"/>
